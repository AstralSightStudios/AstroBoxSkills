...
7. Windows 需要在系统蓝牙设置中点击允许配对（可能会在**右下角弹出通知**，点击进去配对即可）
...
1. 小米运动健康是否**完全清除后台**（或"小米运动健康"的"附近的设备"权限是否处于关闭状态）
2. 表盘自定义工具、Notify For Xiaomi、GadgetBridge 等**其它干扰项的权限均为关闭状态**，必要时可以直接卸载。（澎湃 OS2 用户可以尝试调整"互联互通服务"防止抢连接）
3. 手环是否进入**连接新设备模式**
4. 是否给 AstroBox **"附近的设备"、"蓝牙"、"位置获取"权限**
5. 电脑请检查是否有**蓝牙模块（4.0+）**，检查有没有在**设置点击确认，手环点击确认**
6. 请尝试在系统蓝牙设置中**"忽略"你的手环**，然后尝试重新连接**（iOS 用户尤其需要尝试此步）**。Windows 由于系统特性可以重启电脑再试。
7. 如果**固件有更新版本**请在系统设置里**自行更新**
8. 安卓版本是否在**13 以上**
...
首先检查你的设备是否能在 AstroBox 中使用，详情查看支持列表。
